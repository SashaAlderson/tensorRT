...
int main(int argc, char** argv)
{
    ...
    Parser parserHoldingWeightsMem;
    {
        ...
        std::swap(iEnv.engineBlob, bEnv.engineBlob);
    }
    ...
    if (!options.build.safe && iEnv.engine.get()->isRefittable())
    {
        ...
        if (options.inference.timeRefit)
        {
            ...
            {
                bool const success = timeRefit(*networkForRefit, *iEnv.engine, options.inference.threads);
                if (!success)
                {
    ...
    else
    {
        ...
        // reduce memory usage.
        std::vector<uint8_t>().swap(iEnv.engineBlob);
    }
